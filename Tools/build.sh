...
for attempt in 1 2 3; do
  ...
  "$U" -batchmode -quit -disable-assembly-updater -projectPath "$P" \
       -executeMethod AshenSol.EditorTools.BuildScript.BuildWindows -logFile "$LOG" "$@"
  code=$?
  if grep -q "EPERM" "$LOG"; then
    echo "[build] EPERM package flake on attempt $attempt — retrying"
...
grep -E "\[Build\]|\[Bootstrap\]|Build Finished|BuildFailedException|Error building|Shader error" "$LOG" | head -20 | cut -c1-240
